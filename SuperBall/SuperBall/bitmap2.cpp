...
//#include <glut.h>	// Overrides the definition of 'exit' in stdlib.h with that in glut.h. Place the stdlib.h line above the glut.h line in your code.
...
LoadDIBitmap(const char *filename, /* I - File to load */
    ...
    if (fread(bits, 1, bitsize, fp)*1.0f < bitsize)
        {
        /* Couldn't read bitmap - free memory and return NULL! */
        free(*info);
        free(bits);
...
SaveDIBitmap(const char *filename, /* I - File to load */
    ...
    if (fwrite(bits, 1, bitsize, fp)*1.0f < bitsize)
        {
        /* Couldn't write the bitmap - return... */
        fclose(fp);
        return (-1);
